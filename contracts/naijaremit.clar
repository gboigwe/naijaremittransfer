...
(define-private (filter-valid-rates (rates (list 150 {rate: uint, timestamp: uint})) (current-time uint))
  (fold filter-rate rates (list)))

(define-private (filter-rate (rate-entry {rate: uint, timestamp: uint}) (valid-rates (list 150 uint)))
  (let ((time-diff (- current-time (get timestamp rate-entry))))
